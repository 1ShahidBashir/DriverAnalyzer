...
* **Real-time EMA vs. Batch Aggregation:** Instead of running periodic batch jobs (cron) to aggregate sentiment scores—which risks delayed alerts and database load spikes—the system uses an Exponential Moving Average (EMA). This trades historical recalculation accuracy for real-time efficiency, prioritizing immediate operational awareness.
...
| Layer | Technology |
| --- | --- |
| **Frontend** | React 19, Vite, TypeScript, TailwindCSS, Recharts, Recoil |
...
│       ├── store/           # Recoil atoms (auth + feature flags)
...
| **Dependency Injection** | `FeedbackWorker` receives analyzer & metrics srvc | Testable, loosely coupled components |
...
The system updates driver scores using an EMA to ensure the calculation processes in constant time and space complexity, avoiding heavy table scans of historical data.

* **α = 0.2** — recent scores get 20% weight.
* Only stores the current EMA and total trip count per driver.
...
* Returns **3.0** (neutral) if no sentiment words are found.
